...
header says which one it logs (`Model_log(OR)`, `Model_log(RR)`, `Model_log(RoM)`); where the model
...
The empirical average TV hours per day, and their ratios to the reference, can be computed in a simple table with : 
...
for a risk ratio, an odds ratio or a ratio of means, the plain difference for a mean
...
on the effect's own scale — the log-ratio for an odds ratio, a risk ratio or a ratio of means,
