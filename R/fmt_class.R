# PURPOSE: tabxplor_fmt, the rich-cell vctrs record -- plus the colour engine and the MEASURES that
#   drive it.
# ROLE: Foundation of the package. Every numeric column of a table is an fmt vector; the colour engine
#   here is the one artifact every backend (console / HTML / Excel / Markdown / plot) consumes. What
#   the colours are SAID to mean is not here: the sentence is R/tab-legend.R, the region it prints in
#   is R/tab-footer.R.
# KEY CONSTRAINTS:
...
# === SECTION: the colour MEASURES ==================================================================
# The per-measure fact table and the vocabulary accessors that read it -- every language-invariant
# fact of a colour measure, and the ONE door to it (measure_facts). What these facts are RENDERED
# into lives elsewhere: the SENTENCE in R/tab-legend.R, the REGION it prints in in R/tab-footer.R.

# fixed (non-translated) symbols, kept as \uXXXX so R source stays ASCII. Here rather than beside the
# legend that prints them: MEASURES reads them at SOURCE time, so they must be defined before it.
.lg_ge    <- "\u2265"   # >=
.lg_le    <- "\u2264"   # <=
.lg_times <- "\u00d7"   # x  (times)
.lg_div   <- "\u00f7"   # /  (division)
...
MEASURES <- list(
  ...
  contrib    = list(word = function() gettext("contribution to Chi2"), break_over = .lg_times, break_under = .lg_times,
                    ...
                    # the baseline's NAME, per reading: a share of this table's chi2 is read against
                    # the mean contribution, a standardized residual against independence itself.
                    # ⚠ TWO SHAPES, because a sentence needs two: `ref_word` is the compact clause the
                    # terse form puts in brackets, PREPOSITION INCLUDED; `ref_phrase` is the bare noun
                    # a prose lead points at ("... over-represented vs <ref_phrase>"). A measure that
                    # re-states one almost always re-states the other.
                    ref_word = function() gettext("vs the mean"),
                    ...
                    by_scale = list(zscore = list(break_over = "+", break_under = "-",
                                                  ...
                                                  ref_word = function() gettext("vs independence"),
                                                  ref_phrase = function() gettext("independence")))),
...
# WHICH fields a TABLE may re-state (set_legend_words()). Naming only: never an engine fact (`raw`,
...
# set are not a subset of the ones MEASURES declares (`lead_over` exists only as an override, and
# `word_std` only on a by_scale row). Checked HERE and not in zzz-fact-keys.R, which carries edges
# between tables rather than a table's own consistency.
stopifnot(!any(c("raw", "scale", "scale_from", "sig_source", "bounds", "gate_row", "force_policy",
                 "builds", "requires", "channels", "producers", "applies_to", "auto_for",
                 "break_over", "break_under", "break_scale", "break_origin", "threshold_mult",
                 "unit_kind", "ref_kind", "has_ref_lead", "guar", "by_scale", "lead")
              %in% MEASURE_WORD_FIELDS))

# A WORD as the legend needs it: the DECLARED closure (so gettext() runs at render and potools still
# sees the literal), or the plain string a table re-stated it with. `...` are the sprintf arguments a
# templated word takes (`word_guar` its confidence level, a lead its subject / reference / null).
#' @keywords internal
measure_word_of <- function(v, ...) {
...
measure_facts <- function(measure, policy = "ignore", scale_key = NULL, words = NULL) {
  ...
  # ...and LAST, the table's own words (set_legend_words()): the same fold, one layer further out, so
